(() => {
  ...
  if (!window.L) {
    showMapStatus("No fue posible cargar el motor del mapa. Recarga la página.");
    return;
  }

  const DB = {
    summary:{"registros_encuesta_con_mbot":147,"planteles_unicos":145,"primaria":104,"secundaria":41,"kits_total_estimado":940,"kits_sin_cantidad":1,"geolocalizados":145,"estados":{"Sin uso":125,"En uso":15,"Uso limitado":2,"Requiere capacitación":2,"Dañados / incompletos":1},"duplicados_resueltos":2,"cct_ajustados_para_ubicacion":6},
    schools:Array.isArray(window.MBOT_SCHOOLS) ? window.MBOT_SCHOOLS : []
  };
  const schools = DB.schools;

  if (!schools.length) {
    showMapStatus("No se cargaron los datos de planteles. Recarga la página.");
  } else if (schools.length !== DB.summary.planteles_unicos) {
    showMapStatus(`Se cargaron ${schools.length} de ${DB.summary.planteles_unicos} planteles.`);
  }
  ...
  osm.on("tileerror", () => {
    ...
    if (tileErrors >= 4 && !usingFallback) {
      ...
      carto.addTo(map);
      showMapStatus("Se activó un mapa base alternativo por un problema temporal de carga.");
      setTimeout(() => { if (schools.length === DB.summary.planteles_unicos) showMapStatus(""); }, 5000);
    }
  });
  osm.addTo(map);
  ...
  const els = {
    ...
    kpiSchools:document.getElementById("kpiSchools"),
    kpiKits:document.getElementById("kpiKits"),
    kpiUse:document.getElementById("kpiUse"),
    kpiPrimary:document.getElementById("kpiPrimary"),
    kpiSecondary:document.getElementById("kpiSecondary")
  };
  ...
  function radius(k){
    return k == null ? 6 : Math.max(6, Math.min(17, 5 + Math.sqrt(k) * 2));
  }
  ...
  function renderMap(){
    ...
    filtered.forEach(s => {
      ...
      const m = L.circleMarker([s.lat,s.lng], {
        renderer,
        radius:radius(s.kits),
        color:"#ffffff",
        weight:1.5,
        fillColor:statusColors[s.condicion] || "#68738a",
        fillOpacity:.86,
        bubblingMouseEvents:false
      }).bindPopup(popupHtml(s), {maxWidth:340, autoPanPadding:[30,30]});
  ...
  function renderList(){
    ...
    if(!filtered.length){
      els.results.innerHTML = '<div class="empty">No hay planteles que coincidan con los filtros.</div>';
      return;
    }
  ...
  function selectSchool(cct, move){
    ...
    const s = schools.find(x => x.cct === cct);
    const m = markers.get(cct);
    if(s && m){
      if(move) map.flyTo([s.lat,s.lng], Math.max(map.getZoom(),14), {duration:.55});
      m.openPopup();
    }
  ...
  function fitCurrent(){
    ...
    else map.fitBounds(pts,{padding:[34,34],maxZoom:13,animate:false});
  }
  ...
  [els.q,els.level,els.region,els.municipality,els.condition,els.kits].forEach(el => {
    el.addEventListener(el === els.q ? "input" : "change", applyFilters);
  });
  ...
  document.getElementById("downloadCsv").addEventListener("click", () => {
    ...
    a.download = "mbot_jalisco_filtrado.csv";
    a.click();
  ...
  window.addEventListener("load", () => setTimeout(refreshMap,80), {once:true});
